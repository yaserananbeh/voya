...
import { RenderPropsDataFetcher, RenderPropsList } from '@/components/patterns'
...
export function ResultsListRenderProps() {
  ...
  return (
    <RenderPropsDataFetcher
      data={filteredHotels}
      isLoading={isLoading && allHotels.length === 0}
      isError={isError}
      error={isError ? 'Failed to load hotels' : undefined}
    ...
    >
      {({ data: hotels }) => (
        <Stack spacing={2}>
          <RenderPropsList
            items={hotels}
            renderItem={(hotel) => <HotelResultCard key={hotel.id} hotel={hotel} />}
